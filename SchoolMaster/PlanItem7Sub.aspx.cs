...
public partial class SchoolMaster_PlanItem7Sub : System.Web.UI.Page
{
    ...
    private void getTitle()
    {
        ...
        string query = "select Department from PlanDepartmentOutline where SN='" + Session["UserPlanListSN"].ToString() + "' and NO='" + Request["DepartmentNO"].ToString() + "'";
    ...
    private void setInitial()
    {
        ...
        if (data.Count > 0)
        {
            ...
            for (int i = 0; i < data.Count; i++)
            {
                ...
                ((TextBox)GvSchool.Rows[i].Cells[2].FindControl("column3")).Text = d[2].Contains(BaseClass.standardTimestamp) ? Resources.Resource.TipNotWrite : d[2].Split(' ')[0];
                ((TextBox)GvSchool.Rows[i].Cells[3].FindControl("column4")).Text = d[3].Contains(BaseClass.standardTimestamp) ? Resources.Resource.TipNotWrite : d[3].Split(' ')[0];
    ...
    private void storeData()
    {
        if (ViewState["dt"] != null)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ManageSQL ms = new ManageSQL();
                // 先刪除原本的
                string query = "delete from PlanDepartmentList where SN ='" + Session["UserPlanListSN"].ToString() + "' and DepartmentNO='" + schoolDepartmentNO.ToString() + "'";
                ms.WriteData(query, sb);
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    sb.Clear();
                    query = "insert into PlanDepartmentList (SN, DepartmentNO, Target, Activity, StartTime, EndTime, FinishRate, Condition) VALUES ('" +
