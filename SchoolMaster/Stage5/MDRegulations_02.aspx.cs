...
public partial class Stage5_MDRegulations_02 : System.Web.UI.Page
{
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        if (btn.CommandArgument.Equals("1"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "1" + "." + FileUpload1.FileName.Split('.')[(FileUpload1.FileName.Split('.').Length - 1)];
        ...
        else if (btn.CommandArgument.Equals("2"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "2" + "." + FileUpload2.FileName.Split('.')[(FileUpload2.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
        ...
        else if (btn.CommandArgument.Equals("3"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "3" + "." + FileUpload3.FileName.Split('.')[(FileUpload3.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
        ...
        else if (btn.CommandArgument.Equals("4"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "4" + "." + FileUpload4.FileName.Split('.')[(FileUpload4.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
        ...
        else if (btn.CommandArgument.Equals("5"))
        {
            ...
            if (FileUpload5.HasFile)
            {
                ...
                if (!isFind)
                {
                    //LbStatus5.Text = "上传格式必须为WORD (*.doc, *.docx)";
                    return;
                }
            }
            string saveDir = @"Upload\Stage5\";
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "5" + "." + FileUpload5.FileName.Split('.')[(FileUpload5.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
        ...
        else if (btn.CommandArgument.Equals("6"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "6" + "." + FileUpload6.FileName.Split('.')[(FileUpload6.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
        ...
        else if (btn.CommandArgument.Equals("7"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "7" + "." + FileUpload7.FileName.Split('.')[(FileUpload7.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
        ...
        else if (btn.CommandArgument.Equals("8"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "8" + "." + FileUpload8.FileName.Split('.')[(FileUpload8.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
        ...
        else if (btn.CommandArgument.Equals("9"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "9" + "." + FileUpload9.FileName.Split('.')[(FileUpload9.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
        ...
        else if (btn.CommandArgument.Equals("10"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "10" + "." + FileUpload10.FileName.Split('.')[(FileUpload10.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
        ...
        else if (btn.CommandArgument.Equals("11"))
        {
            ...
            string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + "11" + "." + FileUpload11.FileName.Split('.')[(FileUpload11.FileName.Split('.').Length - 1)];
            string pathToCheck = appPath + saveDir + fileName;
            //===========================================(Start)
            foreach (string file in System.IO.Directory.GetFileSystemEntries(appPath + saveDir))
            {
    ...
    private bool getUploadDir(int targetIndex, StringBuilder sb, StringBuilder outFile)
    {
        string saveDir = @"Upload\Stage5\";
        string appPath = Request.PhysicalApplicationPath;
        string fileName = Session["UserID"].ToString() + DateTime.Now.ToString("yyyy") + "2" + targetIndex;   //目前刪除getSemester()函示不做學期的判斷
        string pathToCheck = appPath + saveDir + fileName;
